#!/usr/bin/env bash
rollout_for_session() {
  local session_id=$1 path meta index_age now
  [[ -d "$SESSIONS_DIR" && "$session_id" =~ ^[A-Za-z0-9._-]+$ ]] || return 1
  mkdir -p "$STATE_DIR" 2>/dev/null || return 1
  now=$(date +%s)
  index_age=999999
  if [[ -f "$ROLLOUT_INDEX" ]]; then index_age=$((now - $(stat -f %m "$ROLLOUT_INDEX" 2>/dev/null || stat -c %Y "$ROLLOUT_INDEX" 2>/dev/null || printf 0))); fi
