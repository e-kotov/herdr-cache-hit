...
CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -trimpath -ldflags='-s -w' -o "$root/bin/agy-usage-darwin-amd64" "$root/cmd/agy-usage"
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -trimpath -ldflags='-s -w' -o "$root/bin/agy-usage-linux-amd64" "$root/cmd/agy-usage"
CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -trimpath -ldflags='-s -w' -o "$root/bin/agy-usage-linux-arm64" "$root/cmd/agy-usage"
